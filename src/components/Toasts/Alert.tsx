  ...
  return (
    <div
      className={`${visible ? "toast-enter" : "toast-leave"
        } max-w-md bg-tertiary-800 rounded-lg flex flex-row p-2 border-2 border-alert-500 min-h-14 gap-4`}
      onAnimationEnd={() => {
        if (!visible) {
          toast.dismiss(t.id);
        }
    ...
    >
      <div className='flex justify-center items-center p-1'>
        <WarningCircle className="w-8 h-8 text-alert-500" size={32} />
      </div>

      <div className='flex flex-col justify-start items-start font-primary'>
        <span className="font-normal text-sm text-alert-100">{topMsg}</span>
        <span className="font-black text-base text-alert-500">{bottomMsg}</span>
      </div>
