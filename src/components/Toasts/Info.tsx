  ...
  return (
    <div
      className={`${visible ? "toast-enter" : "toast-leave"
        } max-w-md bg-neutral-800 rounded-lg flex flex-row p-2 border-2 border-info-500 min-h-14 gap-4`}
      onAnimationEnd={() => {
        if (!visible) {
          toast.dismiss(t.id);
        }
    ...
    >
      <div className='flex justify-center items-center p-1'>
        <PhosphorInfo className="w-8 h-8 text-blue-500" size={32} />
      </div>

      <div className='flex flex-col justify-start items-start font-primary'>
        <span className="font-normal text-sm text-blue-100">{topMsg}</span>
        <span className="font-black text-base text-blue-500">{bottomMsg}</span>
      </div>
